...
"""Insight:
- Terdapat 1000 baris dan 9 kolom
- Tidak ada data yang NaN atau missing value
- Tidak ada data yang duplicated
- User ID memiliki 100 jenis artinya user ID adalah pembeli dan Product ID adalah setiap item dari Product Name

## Data Preparation

Pada tahap ini data dipersiapkan untuk digunakan dalam model machine learning:
"""

fashion.duplicated().sum()

"""Setelah dilakukan pengecekan diketahui bahwa dataset tidak memiliki duplicated data"""

fashion.isnull().sum()

"""Setelah dilakukan pengecekan diketahui bahwa dataset tidak memiliki Missing Value atau NaN"""
...
"""Proses diatas dimaksudkan untuk menyandikan fitur User ID dan Product ID ke dalam indeks integer"""

# Mapping User ID ke dataframe user
fashion['Pembeli'] = fashion['User ID'].map(user_to_user_encoded)

# Mapping Produk ID ke dataframe produk
fashion['Jumlah Produk'] = fashion['Product ID'].map(produk_to_produk_encoded)

"""Proses ini ditunjukkan untuk mapping User ID dan Produk ID ke masing - masing dataframe user, produk"""
...
"""Proses diatas memiliki tujuan untuk mengacak data supaya tidak terurut atau random, supaya data bersifat variatif
...
"""Selain mengelompkkan data menjadi train dan test, code di atas memiliki tujuan untuk menormalisasi fitur Rating (Fitur untuk rekomendasi) supaya model lebih mudah mengenalinya dalam bentuk matriks
...
"""Setelah melakukan training diperoleh loss yaitu 0.6643 dan val l0ss 0.6927, loss ini cukup kecil
